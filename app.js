...
const defaultProjects = [
  {
    title: "Ola Analytics",
    description: "End-to-end Ola ride data analysis using SQL and Power BI, focusing on ride trends, cancellations, revenue insights, and user behavior.",
    tags: ["SQL", "Power BI", "Data Analysis"],
    tagColors: ["", "green", "pink"],
  ...
  },
  {
    ...
    description: "Data analysis project using SQL, Python, and Power BI to extract insights from customer behavior data including segmentation, retention, and purchasing patterns.",
...
function renderProjects() {
  ...
  projects.forEach((proj, idx) => {
    ...
    const tagsHTML = (proj.tags || [])
      .map((t, i) => `<span class="tag ${proj.tagColors?.[i] || colors[i % 3]}">${t}</span>`)
      .join("");

    const imgSrc = proj.image || `https://placehold.co/600x400/1a1a2e/7c5cfc?text=${encodeURIComponent(proj.title)}`;

    card.innerHTML = `
      <div class="project-card-img-wrap">
        <img class="project-card-img" src="${imgSrc}" alt="${proj.title}" loading="lazy"
             onerror="this.src='https://placehold.co/600x400/1a1a2e/7c5cfc?text=${encodeURIComponent(proj.title)}'"/>
      </div>
...
async function fetchGitHubRepo() {
  ...
  if (!rawUrl) {
    statusEl.textContent = "⚠️ Please paste a GitHub repo URL";
  ...
  if (!match) {
    statusEl.textContent = "⚠️ Invalid GitHub URL. Use format: https://github.com/user/repo";
    statusEl.style.color = "var(--accent-3)";
    return;
  }
  ...
  try {
    ...
    statusEl.textContent = `✅ Fetched "${data.name}" — review & submit below!`;
    statusEl.style.color = "var(--accent-2)";
  } catch (err) {
...
function handleAddProject(e) {
  ...
  projects.push({
    ...
    tags,
    tagColors: tags.map((_, i) => colors[i % 3]),
